...
class Tristatecr_Listing_Api
{
    ...
    public function register_rest_routes()
    {
        register_rest_route('tristatectr/v1', '/listings', array(
            'methods' => 'GET',
            'callback' => array($this, 'tristatecr_rest_listings'),
        ));

        register_rest_route('tristatectr/v1', '/brokers', array(
            'methods' => 'GET',
            'callback' => array($this, 'tristatecr_rest_brokers'),
        ));

        register_rest_route('tristatectr/v2', '/brokers', array(
            'methods' => 'GET',
            'callback' => array($this, 'tristatecr_rest_brokers_v2'),
        ));
    }

    public function tristatecr_has_private_data_access()
    {
    ...
    public function tristatecr_rest_listings($data)
    {
    ...
    public function tristatecr_rest_brokers()
    {
        $brokers = get_option('tristatecr_datasync_brokers') ?? array();
        return (array) $brokers;
    }

    public function tristatecr_rest_brokers_v2()
    {
        $brokers = get_option('tristatecr_datasync_brokers') ?? array();
        $results = array();
